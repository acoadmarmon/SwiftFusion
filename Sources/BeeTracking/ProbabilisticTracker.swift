...
>(
  ...
  return (fig, track.map{Pose2(Rot2(0.0), $0)}, groundTruth)
}
...
public func runProbabilisticTracker<Encoder: AppearanceModelEncoder>(
  ...
  withFeatureSize d: Int = 100,
  savePatchesIn resultDirectory: String? = nil
) -> (fig: PythonObject, track: [Vector2], groundTruth: [Pose2]) {
...
) -> TrackingConfiguration<Tuple1<Vector2>> {
...
>(
  ...
  var frameVariableIDs = [Tuple1<TypedID<Vector2>>]()
  for _ in 0..<frames.count {
    frameVariableIDs.append(
      Tuple1(
        variableTemplate.store(Pose2().t)
        ))
  }

  let addPrior = { (variables: Tuple1<TypedID<Vector2>>, values: Tuple1<Vector2>, graph: inout FactorGraph) -> () in
    let (poseID) = unpack(variables)
    let (pose) = unpack(values)
    graph.store(WeightedPriorFactorVector2(poseID, pose, weight: 1e-2, rotWeight: 2e2))
  }

  let addTrackingFactor = { (variables: Tuple1<TypedID<Vector2>>, frame: Tensor<Float>, graph: inout FactorGraph) -> () in
    let (poseID) = unpack(variables)
    graph.store(
  ...
  return TrackingConfiguration(
    ...
    addBetweenFactor: { (variables1, variables2, graph) -> () in
      ...
      graph.store(WeightedBetweenFactorVector2(poseID1, poseID2, Pose2().t, weight: 1e-2, rotWeight: 2e2))
    },
    addFixedBetweenFactor: { (values, variables, graph) -> () in
      let (prior) = unpack(values)
      let (poseID) = unpack(variables)
      graph.store(WeightedPriorFactorVector2SD(poseID, prior, sdX: 8, sdY: 8, sdTheta:0.5))
    })
}
...
//   public var trackingConfiguration: TrackingConfiguration<Tuple1<Vector2>>
...
//   public init(encoder: Encoder, foregroundModel: FG,  backgroundModel: BG, trackingConfiguration: TrackingConfiguration<Tuple1<Vector2>>) {
//     self.encoder = encoder
//     self.foregroundModel = foregroundModel
//     self.backgroundModel = backgroundModel
